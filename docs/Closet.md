...
Users can consolidate duplicate or multi-angle photos of the same item:
...
- **The Gatekeeper Guard**: If target fields (e.g., category, brand, dress code) do not match, the `Gatekeeper` modal alerts the user of inconsistencies before confirming the grouping:
  ...
  ```
  Grouping updates the target item as `group_role: "host"` and the dragged item as `group_role: "member"`.
...
When items are grouped, the background handler `reanalyze_group_helper` calculates a consolidated embedding for the host item. It runs FashionCLIP across all member garment images, merging their visual vectors into a single robust search representation:
