...
* **Responsibility:** Adding garments, background matting (`rembg`), clothing category segmentation (`SegFormer`), DPP QR parsing, and perceptual hashing for duplicate detection.
* **Key Components:** [clothing_parser.py](file:///C:/DressApp_AG/backend/app/services/clothing_parser.py), [background_matting.py](file:///C:/DressApp_AG/backend/app/services/background_matting.py), [dpp_parser.py](file:///C:/DressApp_AG/backend/app/services/dpp_parser.py).
...
| **Credits** | Billing | Pre-purchased virtual currency used to pay for premium stylist actions (such as high-quality outfit generation). |
