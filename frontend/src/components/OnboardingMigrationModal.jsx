...
export default function OnboardingMigrationModal({ isOpen, onClose, onFlagUpdated }) {
  ...
  const harvesterBookmarkletCode = 'javascript:void((async()=>{if(typeof html2canvas===\"undefined\"){const s=document.createElement(\"script\");s.src=\"https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js\";document.head.appendChild(s);await new Promise(r=\u003es.onload=r)}const o=document.createElement(\"div\");o.id=\"dressapp-importer-widget\";o.style.cssText=\"position:fixed;top:20px;left:20px;z-index:999999;background:rgba(15,23,42,0.95);color:white;padding:16px;border-radius:12px;font-family:sans-serif;font-size:13px;box-shadow:0 10px 25px rgba(0,0,0,0.3);width:260px;line-height:1.4;border:1px solid rgba(255,255,255,0.1);\";o.innerHTML=\"\u003cdiv style=\\'font-weight:bold;margin-bottom:8px;font-size:14px;color:#f1f5f9;display:flex;align-items:center;gap:6px;\\'\u003e👗 DressApp Importer\u003c/div\u003e\u003cdiv id=\\'da-status\\' style=\\'color:#94a3b8;font-size:11px;\\'\u003eInitializing...\u003c/div\u003e\";document.body.appendChild(o);const st=document.getElementById(\"da-status\");const getScrollEl=()=\u003e{const common=[\"main\",\"[class*=scroll]\",\"[class*=content]\",\"#root\",\".app-container\"];for(const sel of common){const el=document.querySelector(sel);if(el\u0026\u0026el.scrollHeight\u003eel.clientHeight){const style=window.getComputedStyle(el);if(style.overflowY===\"auto\"||style.overflowY===\"scroll\")return el}}const all=document.querySelectorAll(\"*\");let best=null,maxScroll=0;for(const el of all){const style=window.getComputedStyle(el);if(style.overflowY===\"auto\"||style.overflowY===\"scroll\"){const dist=el.scrollHeight-el.clientHeight;if(dist\u003emaxScroll){maxScroll=dist;best=el}}}return best||document.scrollingElement||document.documentElement||document.body};const scrollEl=getScrollEl();let sc=[];let max=35;let step=350;let prev=-1;for(let i=0;i\u003cmax;i++){if(scrollEl===window||scrollEl===document.body||scrollEl===document.documentElement){window.scrollTo(0,i*step)}else{scrollEl.scrollTop=i*step}await new Promise(r=\u003esetTimeout(r,1000));const currentScroll=(scrollEl===window||scrollEl===document.body||scrollEl===document.documentElement)?window.scrollY:scrollEl.scrollTop;if(currentScroll===prev){st.innerText=\"Bottom reached.\";break}prev=currentScroll;st.innerText=\"Capturing screen \"+(i+1)+\" (pos: \"+currentScroll+\")...\";o.style.display=\"none\";const extWidget=document.getElementById(\"dressapp-importer-widget\");if(extWidget)extWidget.style.display=\"none\";const isWin=scrollEl===window||scrollEl===document.body||scrollEl===document.documentElement;const targetEl=isWin?document.body:scrollEl;const rect=targetEl.getBoundingClientRect();const cropX=rect.left+window.scrollX;const cropY=rect.top+window.scrollY;const cropW=rect.width;const cropH=rect.height;const scrollables=[];if(targetEl.scrollTop\u003e0||targetEl.scrollLeft\u003e0)scrollables.push(targetEl);targetEl.querySelectorAll(\"*\").forEach(el=\u003e{if(el.scrollTop\u003e0||el.scrollLeft\u003e0)scrollables.push(el)});scrollables.forEach((el,idx)=\u003eel.setAttribute(\"data-da-scroll-id\",idx));try{const c=await html2canvas(document.body,{logging:false,useCORS:true,allowTaint:true,x:cropX,y:cropY,width:cropW,height:cropH,scrollX:window.scrollX,scrollY:window.scrollY,windowWidth:window.innerWidth,windowHeight:window.innerHeight,onclone:(clonedDoc)=\u003e{scrollables.forEach((el,idx)=\u003e{const clonedEl=clonedDoc.querySelector(\"[data-da-scroll-id=\\'\"+idx+\"\\']\");if(clonedEl){clonedEl.scrollTop=el.scrollTop;clonedEl.scrollLeft=el.scrollLeft}})}});sc.push(c.toDataURL(\"image/jpeg\",0.75))}catch(e){st.innerText=\"Error: \"+e.message}scrollables.forEach(el=\u003eel.removeAttribute(\"data-da-scroll-id\"));o.style.display=\"block\";if(extWidget)extWidget.style.display=\"flex\"}st.innerText=\"Sending \"+sc.length+\" screens...\";document.dispatchEvent(new CustomEvent(\"DRESSAPP_BOOKMARKLET_SCREENSHOTS\",{detail:{screenshots:sc}}));let sent=false;if(window.opener){try{window.opener.postMessage({type:\"DRESSAPP_MIGRATION_SCREENSHOTS\",screenshots:sc},\"*\");st.innerHTML=\"\u003cspan style=\\'color:#34d399;font-weight:bold;\\'\u003e✓ Done!\u003c/span\u003e Return to DressApp.\";sent=true;setTimeout(()=\u003eo.remove(),4000)}catch(e){}}if(!sent){st.innerHTML=\"\u003cdiv style=\\'margin-bottom:8px;color:#f87171;\\'\u003eOpener lost. Click copy below, go back to DressApp and press Ctrl+V to import.\u003c/div\u003e\u003cbutton id=\\'da-copy-btn\\' style=\\'background:#6366f1;color:white;border:none;padding:6px 12px;border-radius:6px;cursor:pointer;font-weight:bold;font-size:11px;width:100%;\\'\u003eCopy Import Data\u003c/button\u003e\";const cb=document.getElementById(\"da-copy-btn\");if(cb){cb.onclick=()=\u003e{navigator.clipboard.writeText(JSON.stringify({type:\"DRESSAPP_MIGRATION_SCREENSHOTS\",screenshots:sc}));cb.innerText=\"Copied! Press Ctrl+V in DressApp\";cb.style.background=\"#10b981\"}}}})());';
