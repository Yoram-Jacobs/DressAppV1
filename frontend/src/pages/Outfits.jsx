...
export default function Outfits() {
  ...
  const deleteOutfit = async (id) => {
    try {
      ...
      toast.success("Outfit removed from your diary.");
    } catch (err) {
  ...
  return (
    <div className="container-px max-w-6xl mx-auto pt-6 md:pt-10 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
        <div>
          <div className="caps-label text-muted-foreground">My Wardrobe Diary</div>
          <h1 className="font-display text-3xl sm:text-4xl mt-1">Saved Outfits</h1>
          <p className="text-sm text-muted-foreground mt-2 max-w-xl">
            View outfits you have composed and scheduled. Your AI Stylist ensures you utilize all closet assets and warns you of previous occasion repetitions.
          </p>
        </div>
        ...
        <CardContent className="p-5 space-y-4">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-display text-lg font-medium">Notification Center (Simulated)</h3>
            </div>
          ...
          {notifLoading ? (
            <p className="text-xs text-muted-foreground animate-pulse">Loading mock push notifications...</p>
          ) : notifications.length === 0 ? (
            <div className="text-xs text-muted-foreground flex items-center gap-2 p-3 bg-card/40 rounded-xl border border-dashed border-border/60">
              ...
              <span>No notifications triggered yet. Set your daily reminder in settings or schedule an event to test.</span>
            </div>
          ) : (
      ...
      {loading ? (
        ...
        <Card className="rounded-2xl border border-dashed border-border py-16 text-center">
          <CardContent className="space-y-4">
            ...
            <h2 className="font-display text-xl">No outfits saved yet</h2>
            <p className="text-sm text-muted-foreground max-w-sm mx-auto">
              Get outfit proposals in the AI Stylist tab, pick your favorite, and save it to start logging your outfits.
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {outfits.map((o) => {
            // Map garments array to outfitItems object expected by AvatarViewer
              ...
              <Card key={o.id} className="rounded-2xl border border-border bg-card shadow-editorial overflow-hidden flex flex-col group hover:shadow-lg transition-shadow">
                ...
                <div className="relative w-full aspect-[4/5] bg-secondary/10 shrink-0">
                  ...
                  <Badge className="absolute top-3 left-3 rounded-full caps-label bg-background/90 text-foreground border border-border backdrop-blur">
                    {o.source_workflow === 'scheduled' ? 'Scheduled Preset' : 'Special Event'}
                  </Badge>
                  <Button
                    size="icon"
                    variant="destructive"
                ...
                <CardContent className="p-5 flex-1 flex flex-col justify-between space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <div className="caps-label text-[10px] text-muted-foreground">Outfit Pieces</div>
                    <div className="flex flex-wrap gap-1.5">
                      {o.garments.map((g, idx) => (
                        ...
                        >
                          <span className="text-muted-foreground uppercase text-[8px] tracking-wider mr-1">{g.role}</span>
                          <span>{g.title || 'Garment'}</span>
                        </div>
                      ))}
                    </div>
                  </div>
