...
import { ShieldAlert, Coins, TrendingUp } from 'lucide-react';
...
export function QuotaMonitor({ quotaStatus, pricingData }) {
  ...
  const quotaTitle = useMemo(() => {
    switch (quotaStatus.status) {
      case 'soft_warning': return t('pricing.alerts.soft_warning', { defaultValue: 'Running Low' });
      case 'hard_limit': return t('pricing.alerts.hard_limit', { defaultValue: 'Credits Exhausted' });
      case 'exhausted': return t('pricing.alerts.exhausted', { defaultValue: 'No Credits' });
      default: return t('pricing.alerts.all_set', { defaultValue: 'All Set' });
    }
  }, [quotaStatus.status, t]);

  const needsPurchaseLink = ['soft_warning', 'hard_limit', 'exhausted'].includes(quotaStatus.status);

  return (
    <>
      {/* Quota Status Alert Banner */}
      {quotaStatus.status && quotaStatus.status !== 'normal' && (
        <div className={`mb-8 p-4 rounded-2xl border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 ${
          quotaVariant === 'danger' 
          ...
          {needsPurchaseLink && (
            <Link to="/pricing/purchase" className="shrink-0 w-full sm:w-auto" data-testid="pricing-alert-purchase-link">
              <Badge variant="destructive" className="cursor-pointer hover:bg-destructive/90 transition-colors">
