...
def generate_fallback_advice(
    ...
    for top in tops:
        ...
        for bottom in bottoms:
            ...
            for chosen_shoe in (shoes if shoes else [None]):
                ...
                top_clean = resolve_garment_image_url(top)
                bottom_clean = resolve_garment_image_url(bottom)
                ...
                if chosen_shoe:
                    shoe_clean = resolve_garment_image_url(chosen_shoe)
                    items.append({
                        "role": "shoes",
                ...
                if chosen_acc:
                    acc_clean = resolve_garment_image_url(chosen_acc)
                    items.append({
                        "role": "accessory",
    ...
    for dress in dresses:
        ...
        for chosen_shoe in (shoes if shoes else [None]):
            ...
            dress_clean = resolve_garment_image_url(dress)
            items = [
                {
                    "role": "dress",
...
async def generate_event_proposals(
    ...
    try:
        ...
        res_json = generate_fallback_advice(prioritized_closet, style_dress_for=event_prompt)
