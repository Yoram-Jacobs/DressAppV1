...
Bridges Hebrew <-> English (and other common synonyms) so queries in either language
match listings and items tagged in either language.
"""
...
def get_search_needles(query: str | None) -> list[str]:
    """Return a list of query terms expanded with fashion & bilingual synonyms."""
    if not query:
        return []
    ...
    if not q:
        return []
    needles: set[str] = {q}
    if q in FASHION_SYNONYMS:
        needles.update(s.lower() for s in FASHION_SYNONYMS[q])
    words = re.split(r"[\s,;+]+", q)
    for w in words:
        if not w:
            continue
        needles.add(w)
        if w in FASHION_SYNONYMS:
            needles.update(s.lower() for s in FASHION_SYNONYMS[w])
