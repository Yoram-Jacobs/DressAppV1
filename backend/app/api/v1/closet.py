...
async def _run_background_matte(
    ...
    # 4. Auto-crop the final PNG to its non-transparent bounds so the
    #    closet thumbnail focuses tightly on the garment. Solves the
    #    "full un-cropped photo" bug for deferred one-pass items.
    if result:
        try:
            ...
            if bbox:
                ...
                from app.services.garment_vision import _fit_crop_to_card
                out_bytes, out_mime = _fit_crop_to_card(
                    buf.getvalue(), crop_mime="image/png"
                )
                logger.info(
                    "Background matte auto-cropped and fitted to card for item %s", item_id
                )
                result = out_bytes
        except Exception as exc:  # noqa: BLE001
            logger.info("Failed to auto-crop clean image for item %s: %s", item_id, exc)
...
async def create_item(
    ...
    if needs_bg_matte:
        ...
        if payload.crop_base64:
            # We used to decode crop_base64 here to try and run the matte
            # on the tight crop, but rembg (U^2-net) fails when the crop is
            # surrounded by the 900x1200 white canvas generated by the
            # frontend. So we revert to passing the full raw photo to the
            # matte runner, and we will auto-crop its output instead.
            pass
