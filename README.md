...
| **Auto-fill & Re-analyse** | Falls back to local SegFormer + rembg + Gemini. The **Re-analyse Photo** card features an interactive AI prompt box allowing users to ask **The Eyes** for specific modifications (*"Remove the shoes"*, *"Complete the hole where the hand was"*, *"Remove metal studs"*) executing photorealistic inpainting via **Nano Banana** (`gemini-2.5-flash-image`). |
