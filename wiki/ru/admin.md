...
- **Интеграция с Google OAuth**: Авторизация в производственной среде выполняется через Google OAuth (`dressappdeveloper@gmail.com`), что исключает небезопасные локальные методы обхода.
