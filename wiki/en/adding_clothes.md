...
> **Module:** `frontend/src/pages/AddItem.jsx` (4,391 lines)
...
> **Last Updated:** July 2026 (Phase R - Background Polishing & Instant Save)
...
graph TD
    ...
    subgraph UI_Live_Refinement ["UI — Background Polishing"]
        Cards -->|status='ready' & deferMatte=true| PolishHook[useEffect Auto-Matting]
        PolishHook -->|POST /closet/polish-crop| PolishAPI[polishCrop Endpoint]
        PolishAPI -->|refined PNG base64| UpdateCard[Update Card Preview & clear deferMatte]
        UpdateCard -.->|re-trigger on Category change| PolishHook
    end
    ...
    API -->|sync WebP thumb pre-generation| ThumbGen[ensure_thumbnail_and_placeholder]
...
- **Live Background Polishing on Review Screen**: As soon as /analyze yields the initial rough crop, the review screen triggers a background refinement task (`polishCrop`) using a non-blocking `useEffect`. The card displays a subtle loading badge (`"Polishing photo..."`) and updates the preview live once the clean, borderless PNG cutout is ready, without waiting for the user to hit Save.
- **Category-Aware Dynamic Re-Refinement**: If the user corrects or changes the garment's category in the Basic Info dropdown, the page automatically re-fires the background polishing task with the new category. This forces the SegFormer segmenter to update its targeting mask (e.g. switching from Top mask to Bottom mask), ensuring the cutout boundary matches the corrected garment class.
- **Zero-Lag Instant Save**: Users do not have to wait for background polishing to finish before saving. Clicking Save immediately inserts the item. If a background refinement task is still pending, the backend queues a background matting task and processes it asynchronously.
- **Synchronous WebP Thumbnail Generation**: To avoid empty cards or grey flashes on the closet grid right after saving, the backend's `POST /closet` creation endpoint generates the WebP thumbnail and placeholder data URLs synchronously before persisting. The closet page receives a fully rendered thumbnail immediately upon optimistic ghost resolution.
...
#### A.1 File & URL Selection

Four entry points ingest images into the refine phase:

| Control | Action | Details / API |
|---|---|---|
| **Take Photo** (`openCamera`) | Input `<input capture="environment">` | Triggers rear camera on mobile; bypasses chooser on Chrome Android; funnels to `handleFiles`. |
| **Upload Photos** (`pickFiles`) | Input `<input accept="image/*" multiple>` | Enables multi-select; bypasses Photo/Files intent chooser; funnels to `handleFiles`. |
| **Drag & Drop** | Dropzone `onDrop` | Passes files directly to `handleFiles`. |
| **URL Upload** (`handleUrlUpload`) | Modal URL text input | Calls `api.fetchImageUrl(url)` to retrieve the base64 string and MIME type, then instantiates a card directly. |
...
#### A.4.1 Live Background Polishing & Category Adaptation (Lines 367–423)

To let users preview high-quality matted cutouts before saving, `AddItem.jsx` runs an automated background refinement loop driven by a reactive `useEffect`:

1. **Auto-Trigger**: Once an ingest card transitions to `status: 'ready'`, contains `cropBase64`, and has `deferMatte: true` (indicating it has not yet been refined), the hook selects the card, marks it with `polishStatus: 'pending'`, and fires a POST request to `/api/v1/closet/polish-crop` in the background.
2. **Visual Feedback**: While polishing is in-flight, the card renders a spinning `Loader2` alongside a `"Polishing photo…"` badge on the bottom-start corner of the image canvas.
3. **Cutout Hydration**: Upon API success, `previewUrl` and `cropBase64` are updated with the returned high-quality transparent PNG. The `deferMatte` flag is cleared (`false`), ensuring the refined cutout is saved directly to the database.
4. **Category Adaptation**: If the user corrects the garment's category in the Basic Info dropdown (e.g. changing from "Top" to "Bottom"), the hook detects that the selected category differs from `lastPolishedCategory` and re-triggers the polishing endpoint. The SegFormer model dynamically swaps target mask categories on the fly, producing a clean, neck-and-skin-excluded cutout aligned with the corrected garment class.
5. **Non-blocking Save**: The save command remains completely active during polishing. If saved early while polishing is `'pending'`, the backend takes over and queues the matting task as a background task. If saved after polishing is `'success'`, the clean cutout is stored instantly without further pipeline steps.
...
1. User taps the **Scan QR (DPP)** button (represented by the QrCode icon in the **Camera & Upload** tab's action list) → `setScanOpen(true)` → `<DppScanner>` component renders.
...
### 3.7.1 Backend — `/closet/polish-crop` Endpoint (Lines 2110–2133)

This endpoint provides a non-blocking way for the frontend to request immediate matting of a crop.

- **Request Schema** (`PolishCropIn`):
  ```json
  {
  ...
  ```
- **Concurreny & Refinement**: The endpoint runs `remove_background` and SegFormer `parse_garments` in parallel using `asyncio.gather` on the CPU/GPU server. The output mask from `_pick_segformer_mask_for_category` is applied via `apply_alpha_intersection` to produce a refined transparent PNG.
- **Response**: Returns the refined PNG string fully prefixed:
  ```json
  {
    "image_base64": "data:image/png;base64,<base64-data>",
    "applied": true
  }
  ```

### 3.7.2 Backend — Double-Prefix Mitigation (Lines 1084–1102)

To prevent base64 data URLs generated by `polishCrop` or local canvas exports from being double-prefixed during creation (`POST /closet`), the server inspects the incoming payload strings:
```python
if payload.crop_base64:
    if payload.crop_base64.startswith("data:"):
        ...
        doc["segmented_image_url"] = f"data:{_mime};base64,{payload.crop_base64}"
```
This ensures the stored `segmented_image_url` is always a valid browser-renderable data URL (e.g., `data:image/png;base64,...` rather than `data:image/jpeg;base64,data:image/png;base64,...`).

### 3.7.3 Backend — Synchronous WebP Thumbnail & Placeholder Pre-generation (Lines 1323–1335)

To prevent visual empty/grey cards when the frontend reconciles its optimistic store ghosts with the server-returned items, the creation endpoint pre-generates the card's low-resolution WebP thumbnail and placeholder before returning:
```python
try:
...
```
This runs in $\approx 5\text{ ms}$ on the hot path, ensuring the server response carries populated thumbnails that the frontend renders instantly.
