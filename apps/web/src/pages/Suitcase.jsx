  ...
  if (closetItems && closetItems.length > 0) {
    ...
    if (desc) {
      const match = closetItems.find((i) => {
        ...
        return (
          (title && (desc.includes(title) || title.includes(desc))) ||
...
function OutfitCanvas({
    ...
    return (
      ...
      >
        ...
        <span className="text-xs">
          {t("suitcase.noImages", { defaultValue: "No images available" })}
        </span>
      </div>
    );
  }
  ...
  return (
    <div
      className={`relative aspect-[4/3] w-full bg-muted/20 border-b border-border overflow-hidden cursor-pointer hover:opacity-95 transition-opacity ${className}`}
      onClick={handleClick}
    >
      ...
      <div className="absolute inset-0 bg-black/0 hover:bg-black/10 transition-all flex items-center justify-center opacity-0 hover:opacity-100 z-20">
        <div className="bg-white text-[12px] text-dark-brand font-bold px-2 py-1 rounded-full shadow-md flex items-center gap-1.5 pointer-events-auto">
...
function Suitcase() {
  ...
  const handleDeleteArchives = async () => {
    ...
    try {
      ...
      toast.success(
        t("suitcase.archivesDeleted", {
          defaultValue: "Archives deleted successfully.",
        }),
      );
    } catch (e) {
      toast.error(
        t("suitcase.archiveDeleteError", {
          defaultValue: "Failed to delete archives.",
        }),
      );
    } finally {
  ...
  const alreadyPackedIds = useMemo(() => {
    const list =
      viewState === "active"
        ? activeSuitcase?.packing_list
        : packingData?.packing_list;
  ...
  const groupedActiveList = useMemo(() => {
    if (!activeSuitcase || !Array.isArray(activeSuitcase.packing_list))
      return {};
    const groups = {};
  ...
  const autoSavePlanningState = async () => {
    ...
    if (!state.destinations.trim() && !state.departureTime && !state.returnTime)
      return;

    const formattedDeparture = state.departureTime
      ? state.departureTime.includes("T")
        ? state.departureTime
        : `${state.departureTime}T00:00:00`
      : "";
    const formattedReturn = state.returnTime
      ? state.returnTime.includes("T")
        ? state.returnTime
        : `${state.returnTime}T23:59:59`
      : "";
      ...
      const res = await api.saveSuitcaseActive({
        ...
        packing_list:
          state.viewState === "reviewing"
            ? state.packingData?.packing_list || []
            : [],
        outfits:
          state.viewState === "reviewing"
            ? state.packingData?.outfits || []
            : [],
        missing_notes:
          state.viewState === "reviewing"
        ...
        local_fashion_stores:
          state.viewState === "reviewing"
            ? state.packingData?.local_fashion_stores || []
            : [],
      });
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (res.destinations) {
        ...
        updatedDest = res.destinations;
      }
      if (res.purpose) {
        setPurpose(res.purpose);
        updatedPurpose = res.purpose;
      }
      if (res.preferred_style) {
        setPreferredStyle(res.preferred_style);
        updatedPref = res.preferred_style;
      }
      if (res.departure_time) {
        setDepartureTime(res.departure_time);
        updatedDep = res.departure_time;
      }
      if (res.return_time) {
        setReturnTime(res.return_time);
        updatedRet = res.return_time;
      }
      if (res.notes) {
        setNotes(res.notes);
        updatedNotes = res.notes;
      }
      ...
      if (res.should_regenerate) {
        ...
        if (viewState === "gathering") {
          ...
          setChatLoading(true);
          try {
            const formattedDeparture = overrides.departure_time
              ? overrides.departure_time.includes("T")
                ? overrides.departure_time
                : `${overrides.departure_time}T00:00:00`
              : "";
            const formattedReturn = overrides.return_time
              ? overrides.return_time.includes("T")
                ? overrides.return_time
                : `${overrides.return_time}T23:59:59`
              : "";
            ...
            const saveRes = await api.approveSuitcase({
              ...
              missing_notes:
                packed.danger_zones_info || packed.cultural_guidelines || "",
            ...
            if (saveRes.status === "success") {
              ...
              toast.success(
                t("suitcase.activeRefined", {
                  defaultValue: "Suitcase outfits & packing list updated!",
                }),
              );
            }
          } catch (err) {
            toast.error(
              t("suitcase.activeRefineError", {
                defaultValue: "Failed to update outfits.",
              }),
            );
          } finally {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          text: t("suitcase.chatError", {
            defaultValue:
              "Sorry, I had trouble parsing that. Please try typing again!",
          }),
        },
      ]);
    } finally {
  ...
  const handlePack = async (overrides = {}) => {
    const dest =
      overrides.destinations !== undefined
        ? overrides.destinations
        : destinations;
    const purp = overrides.purpose !== undefined ? overrides.purpose : purpose;
    const pref =
      overrides.preferred_style !== undefined
        ? overrides.preferred_style
        : preferredStyle;
    const dep =
      overrides.departure_time !== undefined
        ? overrides.departure_time
        : departureTime;
    const ret =
      overrides.return_time !== undefined ? overrides.return_time : returnTime;
    const n = overrides.notes !== undefined ? overrides.notes : notes;

    if (!dest || !dest.trim()) {
      toast.error(
        t("suitcase.fillDestination", {
          defaultValue: "Please fill in Destinations.",
        }),
      );
      return;
    }
    if (!dep && !ret) {
      toast.error(
        t("suitcase.fillDates", {
          defaultValue: "Please select both Departure and Return dates.",
        }),
      );
      return;
    }
    if (!dep) {
      toast.error(
        t("suitcase.fillDeparture", {
          defaultValue: "Please select a Departure date.",
        }),
      );
      return;
    }
    if (!ret) {
      toast.error(
        t("suitcase.fillReturn", {
          defaultValue: "Please select a Return date.",
        }),
      );
      return;
    }

    const formattedDeparture = dep
      ? dep.includes("T")
        ? dep
        : `${dep}T00:00:00`
      : "";
    const formattedReturn = ret
      ? ret.includes("T")
        ? ret
        : `${ret}T23:59:59`
      : "";
    ...
    try {
      ...
      toast.error(
        t("suitcase.packError", {
          defaultValue:
            "Stylist packing generator failed. Check your API configuration.",
        }),
      );
    } finally {
  ...
  const handleApprove = async () => {
    ...
    const currentOutfits = isEditingActive
      ? activeSuitcase?.outfits
      : packingData?.outfits;
    const currentPackingList = isEditingActive
      ? activeSuitcase?.packing_list
      : packingData?.packing_list;
    const currentMissingNotes = isEditingActive
      ? activeSuitcase?.missing_notes
      : packingData?.danger_zones_info || packingData?.cultural_guidelines;
    const currentLocalStores = isEditingActive
      ? activeSuitcase?.local_fashion_stores
      : packingData?.local_fashion_stores;
    const currentMissingItems = isEditingActive
    ...
    const formattedDeparture = departureTime
      ? departureTime.includes("T")
        ? departureTime
        : `${departureTime}T00:00:00`
      : "";
    const formattedReturn = returnTime
      ? returnTime.includes("T")
        ? returnTime
        : `${returnTime}T23:59:59`
      : "";
    ...
    try {
      ...
      if (res.status === "success") {
        toast.success(
          t("suitcase.approved", {
            defaultValue: "Suitcase approved and packed!",
          }),
        );
      ...
      toast.error(
        t("suitcase.approveError", { defaultValue: "Approve failed" }),
      );
    }
  };
  ...
  const handleRefine = async (feedbackOverride = null, overrides = {}) => {
    const guidance =
      typeof feedbackOverride === "string" ? feedbackOverride : "";
    ...
    try {
      ...
      toast.success(
        t("suitcase.refined", {
          defaultValue: "Packing list refined with your updates!",
        }),
      );
    } catch (e) {
      toast.error(
        t("suitcase.refineError", { defaultValue: "Refinement failed." }),
      );
    } finally {
      setRefining(false);
    }
  ...
  const handleRefineClick = () => {
    setMessages((prev) => [
      ...
      {
        role: "assistant",
        text: t("suitcase.askRefineGuidance", {
          defaultValue: "What would you like to refine in your packing list?",
        }),
      },
    ]);
    setTimeout(() => {
  ...
  const handleDeleteReviewedItem = (id) => {
    if (!packingData || !Array.isArray(packingData.packing_list)) return;
    const itemToDelete = packingData.packing_list.find(
      (item) => item.id === id,
    );
    if (itemToDelete) {
    ...
    const filteredList = packingData.packing_list
      .filter(Boolean)
  ...
  const handleAddFromCloset = (item) => {
    ...
    toast.success(
      t("suitcase.itemAddedFromCloset", {
        defaultValue: "Item added from Closet!",
      }),
    );
  };
  ...
  const handleAddClosetItemToActiveSuitcase = async (item) => {
    ...
    if ((activeSuitcase.packing_list || []).some((p) => p.id === item.id))
      return;
  ...
  const handleTogglePackItem = async (itemId, currentChecked) => {
    ...
    const updatedPackingList = activeSuitcase.packing_list
      .filter(Boolean)
      .map((p) => {
  ...
  const handleDeleteActiveItem = async (itemId) => {
    try {
      ...
      toast.success(
        t("suitcase.itemDeleted", {
          defaultValue: "Item removed from suitcase.",
        }),
      );
      fetchActiveSuitcase();
      ...
      toast.error(
        t("suitcase.deleteItemError", { defaultValue: "Delete item failed." }),
      );
    }
  };
  ...
  const handleUnpack = async () => {
    try {
      const res = await api.deleteSuitcaseActive({
        is_unpack: viewState === "active",
      });
      if (res.status === "success") {
        toast.success(
          t("suitcase.unpackedSuccess", {
            defaultValue: "Welcome back! Suitcase contents moved to Closet.",
          }),
        );
      ...
      toast.error(
        t("suitcase.unpackError", { defaultValue: "Unpack failed." }),
      );
    }
  };
  ...
  const handleSimulateLocation = async () => {
    ...
    try {
      ...
      if (res.status === "success") {
        toast.info(
          t("suitcase.locationEnteredMsg", {
            ...
            isDanger: res.analysis.is_danger_zone
              ? t("common.yes", { defaultValue: "Yes" })
              : t("common.no", { defaultValue: "No" }),
            isHoly: res.analysis.is_holy_place
              ? t("common.yes", { defaultValue: "Yes" })
              : t("common.no", { defaultValue: "No" }),
            defaultValue:
              "Location entered: {{location}}. Danger Zone: {{isDanger}}, Holy Place: {{isHoly}}",
          }),
        );
      ...
      toast.error(
        t("suitcase.simulationError", { defaultValue: "Simulation failed." }),
      );
    } finally {
      setSimulating(false);
    }
  ...
  const countdownText = useMemo(() => {
    ...
    try {
      ...
      if (now < dep) {
        ...
        return t("suitcase.departsIn", {
          count: diff,
          defaultValue: "Departs in {{count}} days",
        });
      } else if (now >= dep && now <= ret) {
        const diff = Math.ceil((ret - now) / (1000 * 3600 * 24));
        return t("suitcase.travelingDays", {
          count: diff,
          defaultValue: "Traveling: {{count}} days remaining",
        });
      } else {
        return t("suitcase.tripCompleted", {
          defaultValue: "Trip completed. Unpacking ready.",
        });
      }
    } catch (e) {
      return "";
  ...
  if (loadingInitial) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] gap-3">
        ...
        <p className="text-sm text-text-brand font-medium">
          {t("suitcase.loadingSuitcase", {
            defaultValue: "Loading your suitcase...",
          })}
        </p>
      </div>
    );
            ...
            <div className="max-w-[520px]">
              ...
              >
                {t("suitcase.subtitleText", {
                  defaultValue:
                    "Traveling AI modular planner and safety advisor.",
                })}
              </p>
            </div>
          </div>
      ...
      <section className="px-[40px] py-[40px] bg-[var(--accent-beige)]">
          ...
          <TabsContent value="suitcase">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-3">
                    ...
                    <Card className="rounded-[12px] border border-border bg-white overflow-hidden">
                      <CardContent className="p-6">
                        <div className="">
                          ...
                          <p className="text-[14px] font-semibold text-text-brand mb-4">
                            {t("suitcase.planNewTripDesc", {
                              defaultValue:
                                "Enter details to curate your custom travel outfit checklist.",
                            })}
                          </p>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="">
                            <label className="text-[14px] font-semibold mb-[8px] text-text-brand block">
                              {t("suitcase.destinationsLabel", {
                                defaultValue: "Destinations",
                              })}
                            </label>
                            <Input
                              placeholder={t(
                                "suitcase.destinationsPlaceholder",
                                {
                                  defaultValue:
                                    "e.g. Rome, Vatican City, Tehran",
                                },
                              )}
                              value={destinations}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="">
                            ...
                            <Input
                              type="date"
                              value={
                                returnTime && typeof returnTime === "string"
                                  ? returnTime.split("T")[0]
                                  : ""
                              }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="">
                            <label className="text-[14px] font-semibold mb-[8px] text-text-brand block">
                              {t("suitcase.preferredStyleLabel", {
                                defaultValue: "Preferred Style",
                              })}
                            </label>
                            <Input
                              placeholder={t("suitcase.stylePlaceholder", {
                                defaultValue:
                                  "e.g. casual modesty, smart-casual, chic",
                              })}
                        ...
                        <div className="">
                          <label className="text-[14px] font-semibold mb-[8px] text-text-brand block">
                            {t("suitcase.tripNotesLabel", {
                              defaultValue: "Trip Notes & Activity Guidelines",
                            })}
                          </label>
                          <Textarea
                            placeholder={t("suitcase.notesPlaceholder", {
                              defaultValue:
                                "e.g. attending gala on day 2, beach activities, mosque visit planned",
                            })}
                            value={notes}
                        ...
                        >
                          {packingLoading ? (
                            ...
                            <>
                              ...
                              {t("suitcase.packButton", {
                                defaultValue: "Pack Suitcase",
                              })}
                            </>
                          )}
                        </Button>
                          ...
                          >
                            <AccordionTrigger className="hover:no-underline">
                              <div className="flex items-center gap-2">
                                <Wand2 className="h-4 w-4 text-primary-brand" />
                                <span className="text-[14px] font-bold text-primary-brand">
                                  {t("suitcase.proposedOutfits", {
                                    defaultValue: "Proposed Daily Outfits",
                                  })}
                                </span>
                              </div>
                            </AccordionTrigger>
                                  ...
                                  .map((outfit, idx) => (
                                    ...
                                    >
                                      ...
                                      <CardContent>
                                        ...
                                        <p className="text-[12px] font-semibold italic text-text-brand p-3">
                                          {outfit.reasoning}
                                        </p>
                                      </CardContent>
                                    </Card>
                              ...
                              <div className="flex items-center justify-between w-full pe-4">
                                ...
                                >
                                  {t("suitcase.itemsCount", {
                                    count:
                                      packingData?.packing_list?.length || 0,
                                        ...
                                        <div className="flex justify-between items-center pb-1 border-b border-border mb-2">
                                          ...
                                          >
                                            ...
                                            {t("suitcase.addFromClosetBtn", {
                                              defaultValue: "Add from Closet",
                                            })}
                                          </Button>
                                        </div>
                                        <div className="">
                                          {items.map((item) => {
                                            const closetMatch = findClosetMatch(
                                              item,
                                              closet.items,
                                            );
                                            return (
                                              ...
                                              >
                                                <div className="flex items-center gap-3">
                                                  ...
                                                  <div className="flex flex-col">
                                                    ...
                                                    <div
                                                      className={`${closetMatch ? "cursor-pointer hover:underline text-primary" : ""}`}
                                                      onClick={() =>
                                                        closetMatch &&
                                                        navigate(
                                                          `/closet/${closetMatch.id}`,
                                          ...
                                          <div className="mt-4 p-3 bg-muted/30 border border-dashed border-border rounded-xl">
                                            <p className="text-xs font-semibold text-muted-foreground mb-2 flex items-center gap-1.5">
                                              ...
                                              {t(
                                                ...
                                                {
                                                  defaultValue:
                                                    "Suggested replacements from your closet:",
                                                },
                                              )}
                                            </p>
                      ...
                      <Card className="rounded-2xl border border-border shadow-sm bg-card">
                        <CardContent className="p-4 flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-base font-semibold truncate max-w-[220px]">
                              {activeSuitcase.destinations}
                            </p>
                          </div>
                        </CardContent>
                      </Card>
                        ...
                        <CardContent className="p-4 flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-base font-semibold capitalize">
                              {t(`suitcase.purpose_${activeSuitcase.purpose}`, {
                                defaultValue: activeSuitcase.purpose,
                              })}{" "}
                              · {activeSuitcase.preferred_style}
                            </p>
                          </div>
                        </CardContent>
                      </Card>
              ...
              <div className="lg:col-span-5">
                <Card className="h-full flex flex-col rounded-[12px] border border-border bg-white overflow-hidden">
                  {/* Header */}
                  <div className="shrink-0 p-6 border-b border-border">
                    <h6 className="text-[16px] font-bold text-dark-brand">
                      <i className="fa-solid fa-wand-sparkles text-primary-brand mr-2"></i>
                      {t("suitcase.chatHeader", {
                        defaultValue: "Suitcase Assistant Chat",
                      })}
                    </h6>
                    ...
                    <p className="text-[14px] font-semibold text-text-brand">
                      {t("suitcase.chatDesc", {
                        defaultValue:
                          "Tell me updates about your trip and I'll adjust the fields.",
                      })}
                    </p>
                  </div>
                  {/* Messages */}
                    ...
                    {chatLoading && (
                      <div className="flex justify-start">
                        <div className="bg-secondary text-secondary-foreground rounded-2xl rounded-tl-none p-3 text-sm flex items-center gap-2">
                          ...
                          {t("suitcase.chatLoading", {
                            defaultValue: "Parsing travel requirements...",
                          })}
                        </div>
                      </div>
                    )}
                  ...
                  <div className="p-5">
                    <form
                      ...
                      className="shrink-0 px-[10px] py-[2px] flex items-center gap-2 rounded-full border border-border bg-accent-beige"
                    >
                      {/* Input */}
                      <Input
                        ref={chatInputRef}
                        placeholder={t("suitcase.chatInputPlaceholder", {
                          defaultValue: "Tell your stylist what you need…",
                        })}
                        value={chatInput}
                        onChange={(e) => setChatInput(e.target.value)}
                        disabled={chatLoading}
                        className="mb-0 !border-none !bg-transparent !shadow-none !outline-none !ring-0 focus:!border-none focus:!outline-none focus:!ring-0 focus-visible:!outline-none focus-visible:!ring-0"
                      />
                      {/* Send */}
                      <Button
                        type="submit"
                        size="icon"
                        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[var(--primary-color)] p-0 text-white shadow-[var(--primary-shadow)] transition-all duration-300 hover:-translate-y-[2px] hover:bg-[var(--primary-hover)]"
                        disabled={chatLoading}
                      >
                    ...
                    >
                      ...
                      <CardContent className="pt-3">
                        <p className="text-xs text-muted-foreground font-medium capitalize">
                          ...
                          :{" "}
                          {t(`suitcase.purpose_${arch.purpose}`, {
                            defaultValue: arch.purpose,
                          })}{" "}
                          ·{" "}
                          {t("suitcase.preferredStyleLabel", {
                            defaultValue: "Style",
        ...
        <Dialog open={showSimModal} onOpenChange={setShowSimModal}>
          <DialogContent className="rounded-2xl max-w-md">
            <DialogHeader>
              <DialogTitle>
                {t("suitcase.gpsSimulatorHeader", {
                  defaultValue: "Simulate entering danger zone or holy place",
                })}
              </DialogTitle>
              <DialogDescription>
                {t("suitcase.gpsSimulatorDesc", {
                  defaultValue:
            ...
            <div className="py-2">
              <Input
                placeholder={t("suitcase.gpsPlaceholder", {
                  defaultValue:
                    "e.g. Islamic Republic of Iran, St. Peter's Basilica",
                })}
                value={simLocation}
            ...
            <DialogFooter className="flex gap-2">
              <Button variant="ghost" onClick={() => setShowSimModal(false)}>
                {t("common.cancel", { defaultValue: "Cancel" })}
              </Button>
              <Button
                onClick={handleSimulateLocation}
                disabled={simulating}
                className="bg-primary-brand text-white rounded-xl"
              >
                {simulating ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin me-1" />
                    {t("suitcase.simulating", {
                      defaultValue: "Simulating...",
                    })}
                  </>
                ) : (
                  t("suitcase.simulateLocation", {
                    defaultValue: "Simulate Location Entry",
                  })
                )}
              </Button>
            </DialogFooter>
        ...
        <Dialog
          ...
          onOpenChange={(open) => {
            if (!open) setSelectedArchive(null);
          }}
        >
          {selectedArchive && (
              ...
              <div className="space-y-4 py-3">
                <div>
                  ...
                  <p className="text-sm">
                    ...
                    :{" "}
                    <span className="font-semibold capitalize">
                      {selectedArchive.preferred_style}
                    </span>
                  </p>
                        ...
                        >
                          ...
                          <ul className="text-xs space-y-0.5 text-muted-foreground mt-1.5 list-disc list-inside">
                            {(Array.isArray(outfit?.items) ? outfit.items : [])
                              ...
                              .map((it, itIdx) => (
                                <li key={itIdx}>
                                  {labelForRole(it.role, t)}: {it.description}
                                </li>
                              ))}
                          </ul>
                        </div>
        ...
        <Dialog
          ...
          onOpenChange={() => setFullscreenOutfit(null)}
        >
            ...
            {fullscreenOutfit && (
              <>
                ...
                <div className="flex gap-3">
                  {(() => {
                    const items = Array.isArray(fullscreenOutfit?.items)
                      ? fullscreenOutfit.items
                      : [];
                    ...
                    const hasTopOrDress = items.some(
                      (i) => i?.role === "top" || i?.role === "dress",
                    );

                    let hasValidOuterwearImage = false;
                    if (outerwearItem) {
                      const match = findClosetMatch(
                        outerwearItem,
                        closet.items,
                      );
                      ...
                      return (
                        <>
                          <div className="rounded-2xl border border-border overflow-hidden bg-muted/10 relative flex items-center justify-center shadow-sm w-full">
                            <OutfitCanvas
                              outfit={fullscreenOutfit}
                              excludeRoles={["top", "dress"]}
                              className="border-none hover:opacity-100 cursor-default"
                              t={t}
                            />
                          </div>
                          <div className="rounded-2xl border border-border overflow-hidden bg-muted/10 relative flex items-center justify-center shadow-sm w-full">
                ...
                <div className="space-y-4 flex-1">
                  <h4 className="text-[14px] font-bold text-dark-brand">
                    {t("suitcase.outfitPieces", {
                      defaultValue: "Outfit Pieces",
                    })}
                  </h4>
                  <div className="space-y-2.5">
                    {(Array.isArray(fullscreenOutfit?.items)
                      ? fullscreenOutfit.items
                      : []
                    ).map((item, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className="flex items-center justify-between border border-border p-3 gap-1 bg-primary-shadow rounded-[12px]"
                        >
                          <div className="flex items-center gap-3">
                            {bestImageUrl(closetMatch) ? (
                              <img
                  ...
                  {fullscreenOutfit.reasoning && (
                    <div className="">
                      <h4 className="text-[14px] font-bold text-dark-brand">
                        {t("suitcase.stylistNotes", {
                          defaultValue: "Stylist Notes",
                        })}
                      </h4>
                      <p className="text-[12px] font-semibold italic text-text-brand pt-2">
                        {fullscreenOutfit.reasoning}
                      </p>
                    </div>
                  )}
                ...
                <DialogFooter>
                  <Button onClick={() => setFullscreenOutfit(null)}>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-2xl max-w-xl max-h-[80vh] flex flex-col p-6">
        ...
        <div className="space-y-3 my-2">
          <Input
            placeholder={t("suitcase.searchClosetPlaceholder", {
              defaultValue: "Search by title, color, brand...",
            })}
            value={searchQuery}
