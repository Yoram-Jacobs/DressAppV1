...
const AUDIENCE_TARGETS = [
  ...
  { value: "casual", labelKey: "campaigns.create.audience.casual" },
  {
    ...
    labelKey: "campaigns.create.audience.sustainableFashion",
  },
...
const STEPS = [
  { id: "basic", icon: Tag, labelKey: "campaigns.create.steps.basic" },
  {
    id: "promotion",
    icon: Image,
    labelKey: "campaigns.create.steps.promotion",
  },
  ...
  { id: "audience", icon: Users, labelKey: "campaigns.create.steps.audience" },
  {
    id: "notifications",
    icon: Bell,
    labelKey: "campaigns.create.steps.notifications",
  },
...
export default function CreateCampaign() {
      ...
      campaignApi
        ...
        .then((res) => {
          setForm({
            ...
            discount_pct:
              res.discount_pct !== null && res.discount_pct !== undefined
                ? String(res.discount_pct)
                : "",
            ...
            location: {
              ...
              lat:
                res.location?.lat !== null && res.location?.lat !== undefined
                  ? String(res.location?.lat)
                  : "",
              lon:
                res.location?.lon !== null && res.location?.lon !== undefined
                  ? String(res.location?.lon)
                  : "",
            ...
            audience: {
              ...
              age_min:
                res.audience?.age_min !== null &&
                res.audience?.age_min !== undefined
                  ? String(res.audience?.age_min)
                  : "",
              age_max:
                res.audience?.age_max !== null &&
                res.audience?.age_max !== undefined
                  ? String(res.audience?.age_max)
                  : "",
    ...
    return (
      <div className="container-px max-w-xl mx-auto pt-16 text-center">
        <h1 className="font-display text-2xl">
          {t("campaigns.create.notExpert")}
        </h1>
        <p className="text-text-brand mt-2">
          {t("campaigns.create.notExpertBody")}
        </p>
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.detail || t("campaigns.create.saveError"),
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.detail || t("campaigns.create.submitError"),
      );
      setSubmitPhase("idle");
    }
  };
  ...
  const handlePayPalCapture = async () => {
    ...
    try {
      await campaignApi.captureSubmissionOrder(
        campaignId,
  ...
  const buildPayload = () => ({
    ...
    audience: {
      ...form.audience,
      age_min:
        form.audience.age_min !== "" ? Number(form.audience.age_min) : null,
      age_max:
        form.audience.age_max !== "" ? Number(form.audience.age_max) : null,
    },
  });
    ...
    <>
      ...
      <section className="px-[40px] py-[40px] bg-accent-beige">
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <button
                ...
                disabled={i > step}
                className={cn(
                  "flex items-center gap-1.5 px-6 py-3 rounded-full text-[12px] whitespace-nowrap transition-all font-semibold",
                  isActive && "bg-primary-brand text-white",
                  isDone &&
                    "bg-white text-text-brand border border-border",
                  !isActive &&
                    !isDone &&
                    "text-text-brand opacity-50 cursor-not-allowed",
                )}
          ...
          <CardContent className="p-5 space-y-5">
            ...
            {step === 0 && (
              <>
                <div className="space-y-3">
                  <div>
                    <Label>{t("campaigns.create.basic.title")}</Label>
                    <Input
                      value={form.title}
                  ...
                  <div>
                    <Label>{t("campaigns.create.basic.businessName")}</Label>
                    <Input
                      ...
                      placeholder={t(
                        "campaigns.create.basic.businessNamePlaceholder",
                      )}
                      data-testid="create-campaign-business-name"
                    />
                  </div>
                  <div>
                    <Label>{t("campaigns.create.basic.category")}</Label>
                    <Select
                      value={form.category}
                      onValueChange={(v) => setField("category", v)}
                    >
                      <SelectTrigger
                        data-testid="create-campaign-category"
                      >
                        <SelectValue
                          placeholder={t(
                            "campaigns.create.basic.categoryPlaceholder",
                          )}
                        />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    <Label>{t("campaigns.create.basic.shortDescription")}</Label>
                    <Textarea
                      ...
                      placeholder={t(
                        "campaigns.create.basic.shortDescriptionPlaceholder",
                      )}
                  ...
                  <div>
                    <Label>
                      {t("campaigns.create.basic.longDescription")}
                    </Label>
                    <Textarea
                      className="resize-none"
                      rows={5}
                      ...
                      onChange={(e) =>
                        setField("long_description", e.target.value)
                      }
                      placeholder={t(
                        "campaigns.create.basic.longDescriptionPlaceholder",
                      )}
                  ...
                  <div>
                    <Label>
                      {t("campaigns.create.basic.coverImageUrl")}
                    </Label>
                    <Input
                      value={form.cover_image_url}
                      onChange={(e) =>
                        setField("cover_image_url", e.target.value)
                      }
                      placeholder="https://..."
            ...
            {step === 1 && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label>
                      {t("campaigns.create.promotion.discountPct")}
                    </Label>
                    <Input
                      type="number"
                      min={0}
                  ...
                  <div>
                    <Label>
                      {t("campaigns.create.promotion.couponCode")}
                    </Label>
                    <Input
                      value={form.coupon_code}
                      onChange={(e) =>
                        setField("coupon_code", e.target.value.toUpperCase())
                      }
                ...
                <div>
                  <Label>
                    {t("campaigns.create.promotion.saleType")}
                  </Label>
                  <Select
                    value={form.sale_type}
                    onValueChange={(v) => setField("sale_type", v)}
                  >
                    <SelectTrigger
                      data-testid="create-campaign-sale-type"
                    >
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {[
                        ...
                        <SelectItem key={v} value={v}>
                          {t(`campaigns.create.promotion.saleTypes.${v}`)}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <Label htmlFor="limited-time" className="mb-0">
                    {t("campaigns.create.promotion.limitedTimeOffer")}
                  </Label>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label>
                      {t("campaigns.create.promotion.startDate")}
                    </Label>
                    <Input
                      type="date"
                  ...
                  <div>
                    <Label>
                      {t("campaigns.create.promotion.endDate")}
                    </Label>
                    <Input
                      type="date"
                      value={form.end_date}
            ...
            {step === 2 && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label>
                      {t("campaigns.create.location.country")}
                    </Label>
                    <Input
                  ...
                  <div>
                    <Label>
                      {t("campaigns.create.location.city")}
                    </Label>
                    <Input
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label>
                      {t("campaigns.create.location.lat")}
                    </Label>
                    <Input
                      type="number"
                      step="0.0001"
                  ...
                  <div>
                    <Label>
                      {t("campaigns.create.location.lon")}
                    </Label>
                    <Input
                      type="number"
                      step="0.0001"
                ...
                <div>
                  <Label>
                    {t("campaigns.create.location.radiusKm")}
                  </Label>
                  <Select
                    value={String(form.location.radius_km)}
                    onValueChange={(v) =>
                      setField("location.radius_km", Number(v))
                    }
                  >
                    <SelectTrigger
                      data-testid="create-campaign-radius"
                    >
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
            ...
            {step === 3 && (
              <div className="space-y-4">
                <div>
                  <Label>
                    {t("campaigns.create.audience.targets")}
                  </Label>
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          form.audience.targets.includes(value)
                            ...
                            : "bg-white border-border text-text-brand hover:border-primary",
                        )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label>
                      {t("campaigns.create.audience.ageMin")}
                    </Label>
                    <Input
                      type="number"
                      min={0}
                      ...
                      onChange={(e) =>
                        setField("audience.age_min", e.target.value)
                      }
                      placeholder="18"
                  ...
                  <div>
                    <Label>
                      {t("campaigns.create.audience.ageMax")}
                    </Label>
                    <Input
                      type="number"
                      min={0}
                      ...
                      onChange={(e) =>
                        setField("audience.age_max", e.target.value)
                      }
                      placeholder="65"
            ...
            {step === 4 && (
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[14px] text-dark-brand font-bold">
                      {t("campaigns.create.notifications.masterSwitch")}
                    </p>
                    <p className="text-[12px] text-text-brand font-semibold">
                      {t("campaigns.create.notifications.masterSwitchBody")}
                    </p>
                  </div>
                  <Switch
                    id="notif-master"
                    ...
                    onCheckedChange={(v) =>
                      setField("notifications.master_enabled", v)
                    }
                ...
                {form.notifications.master_enabled && (
                  <>
                    <div className="space-y-3">
                      <Label>
                        {t("campaigns.create.notifications.channels")}
                      </Label>
                      {["push", "email"].map((ch) => (
                        <div key={ch} className="flex items-center gap-3">
                          ...
                          />
                          <Label htmlFor={`channel-${ch}`} className="mb-0">
                    ...
                    <div>
                      <Label>
                        {t("campaigns.create.notifications.timing")}
                      </Label>
                      <Select
                        value={form.notifications.timing}
                        onValueChange={(v) =>
                          setField("notifications.timing", v)
                        }
                      >
                        <SelectTrigger
                          data-testid="create-campaign-notif-timing"
                        >
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    {form.notifications.timing === "custom" && (
                      <div>
                        <Label>
                          {t("campaigns.create.notifications.customDatetime")}
                        </Label>
                        <Input
                          type="datetime-local"
            ...
            {step === 5 && (
              <div className="space-y-4">
                <h2 className="font-bold text-[14px] text-dark-brand">
                  {t("campaigns.create.review.title")}
                </h2>
                <div className="space-y-2 text-[12px]">
                  <div className="flex gap-2">
                    <span className="text-dark-brand font-semibold w-32">
                      {t("campaigns.create.basic.title")}:
                    </span>
                    <span className="font-semibold text-text-brand">{form.title}</span>
                  </div>
                  <div className="flex gap-2">
                    <span className="text-dark-brand font-semibold w-32">
                      {t("campaigns.create.basic.businessName")}:
                    </span>
                    <span className="font-semibold text-text-brand">{form.business_name}</span>
                  </div>
                  <div className="flex gap-2">
                    <span className="text-dark-brand font-semibold w-32">
                      {t("campaigns.create.basic.category")}:
                    </span>
                  ...
                  {form.discount_pct && (
                    <div className="flex gap-2">
                      ...
                      <Badge className="bg-primary-brand text-white border-0 rounded-full text-[10px]">
                        {form.discount_pct}%
                      </Badge>
                    </div>
                  )}
                  ...
                  {form.end_date && (
                    <div className="flex gap-2">
                      <span className="text-dark-brand font-semibold w-32">
                        {t("campaigns.detail.until")}:
                      </span>
                      <span className="font-semibold text-text-brand">{form.end_date}</span>
                    </div>
                  )}
                  <div className="flex gap-2">
                    <span className="text-dark-brand font-semibold w-32">
                ...
                {/* Fee breakdown */}
                <div className="rounded-[12px] bg-primary-shadow border border-primary-shadow p-3 mt-4">
                  <div className="flex items-center text-dark-brand">
                    <DollarSign className="h-4 w-4" />
                    <h4 className="font-semibold text-sm">
                      {t("campaigns.billing.totalFee")}
                    </h4>
                  </div>
                  <div className="text-[25px] font-bold text-primary-brand my-2">
                    ${feeInfo.feeDollars} USD
                  </div>
                  <div className="text-[12px] font-semibold text-text-brand">
                    {t("campaigns.billing.feePerDay")} × {feeInfo.days}{" "}
                    {t("campaigns.billing.days", { count: feeInfo.days })}
                  </div>
        ...
        <div className="flex items-center justify-between mt-5 gap-3">
          <Button
            ...
            className="!gap-1"
            onClick={() =>
              step === 0 ? navigate("/campaigns/mine") : setStep(step - 1)
            }
          ...
          <div className="flex gap-2">
            {step === STEPS.length - 1 ? (
              <>
                ...
                {submitPhase === "awaiting_payment" && paypalOrder ? (
                  ...
                  <Button
                    ...
                    disabled={
                      submitPhase !== "idle" || !form.title || !form.category
                    }
                    data-testid="campaign-submit-for-approval-btn"
                  >
                    {submitPhase === "creating_order" && (
                      <Loader2 className="h-4 w-4 animate-spin" />
                    )}
              ...
              <Button
                ...
                disabled={
                  step === 0 &&
                  (!form.title || !form.category || !form.short_description)
                }
      ...
      <Dialog open={paypalRequired} onOpenChange={setPaypalRequired}>
        <DialogContent data-testid="paypal-required-dialog">
            ...
            <DialogDescription>
              {t("campaigns.billing.paypalRequiredBody")}
            </DialogDescription>
          <div className="rounded-[12px] bg-primary-shadow p-3 text-sm text-center">
            <span className="text-[25px] font-bold text-primary-brand">
              ${feeInfo.feeDollars}
            </span>
            <span className="text-text-brand ml-1 text-[12px]">
              USD / campaign
            </span>
          </div>
