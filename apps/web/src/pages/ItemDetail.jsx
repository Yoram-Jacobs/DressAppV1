...
function toFormState(item, user = null) {
  ...
  const size = rawSize || (user ? deriveSizeFromPreferences(user, item) : "");
  return {
    title: item.title || "",
    name: item.name || "",
...
function ChipList({ value, onChange, placeholder, disabled, testidPrefix }) {
  ...
  return (
    <div
      ...
      className="createlisting-chiplist"
    >
      {value.length > 0 && (
...
function NullableSelect({
  ...
  return (
    ...
    >
      <SelectTrigger
        className={`rounded-xl h-10 ${className || ""}`}
        data-testid={testid}
      >
...
export default function ItemDetail() {
  ...
  const onPhotoFileChosen = async (e) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "ItemDetail: closetStore sync after photo upload failed",
          e,
        );
      }
  ...
  const initialHostId = useMemo(() => {
    ...
    const hostItem =
      dbList.find((x) => x.group_role === "host" || x.id === x.group_id) ||
      dbList[0] ||
      item;
  ...
  useEffect(() => {
    if (item) {
      ...
      const hostItem =
        unique.find((x) => x.group_role === "host" || x.id === x.group_id) ||
        unique[0] ||
        item;
  ...
  const currentGroupItems = useMemo(() => {
    ...
    const filteredDbList = dbList.filter(
      (x) => !deletedGroupMemberIds.has(x.id),
    );
  ...
  const handleSelectClosetItem = (targetMemberId) => {
    const targetItem = (closetState.items || []).find(
  ...
  const executeSavePipeline = () => {
    ...
    for (const removeId of deletedGroupMemberIds) {
      const removedItem =
        groupItemsState.find((it) => it.id === removeId) ||
        storeItems.find((it) => it.id === removeId);
      if (removedItem) {
        closetStore.upsert({
          ...removedItem,
          group_id: null,
    ...
    toast.success(
      t("itemDetail.group.savingInBackground", {
        defaultValue: "Saving changes in background...",
      }),
    );
    ...
    const savePromise = (async () => {
      ...
      if (groupIsDirty) {
        const payload = {
          new_host_id:
            hostIdState !== null && hostIdState !== initialHostId
              ? hostIdState
              : null,
  ...
  const onSave = () => {
    ...
    const members = currentGroupItems.filter(
      (x) => x.id !== (hostIdState || id),
    );
  ...
  const onReanalyze = async () => {
    ...
    try {
      ...
      const isReceiptItem =
        item?.from_receipt ||
        (Array.isArray(item?.receipt_locked_fields) &&
          item.receipt_locked_fields.length > 0);
      const res = await api.reanalyzeItem(id, {
        fill_empty_only: isReceiptItem,
      });
      setForm(toFormState(res.item, user));
      toast.success(
        t("itemDetail.reanalyze.success") + " · Press Save to keep changes.",
      );
    } catch (err) {
      toast.error(
  ...
  const onSendReanalyzePrompt = async (customPrompt) => {
    const text = (
      typeof customPrompt === "string" ? customPrompt : reanalyzePrompt
    ).trim();
    ...
    try {
      ...
      if (
        res?.item &&
        (res.action_taken === "metadata_update" || res.updated_fields)
      ) {
        setForm(toFormState(res.item, user));
        toast.success(
          t("itemDetail.reanalyze.success") + " · Press Save to keep changes.",
  ...
  const onCleanBackground = async () => {
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.detail || t("itemDetail.cleanBackground.error"),
      );
    } finally {
      clearInterval(ticker);
        ...
        toast.warning(
          res?.detail ||
            t("item.reshootRejected", {
              defaultValue:
                "Restored photo was rejected — keeping the original.",
            }),
        );
      }
    } catch (err) {
  ...
  const activeViewItem =
    currentGroupItems.find((x) => x.id === activeViewIdState) || item;
  const isViewingHost =
    !activeViewIdState || activeViewIdState === (hostIdState || id);
  const hasNewPreview =
    isViewingHost &&
    ((form.reconstructed_image_url &&
      form.reconstructed_image_url !== item.reconstructed_image_url) ||
  ...
  const mergedItem = {
    ...
    reconstructed_image_url: isViewingHost
      ? form.reconstructed_image_url || activeViewItem?.reconstructed_image_url
      : activeViewItem?.reconstructed_image_url,
    clean_image_url: isViewingHost
      ? form.clean_image_url || activeViewItem?.clean_image_url
      : activeViewItem?.clean_image_url,
    clean_image_status: isViewingHost
      ? form.clean_image_status || activeViewItem?.clean_image_status
      : activeViewItem?.clean_image_status,
  };
  ...
  const showRepairPhotoCta =
    (!!mergedItem.reconstruction_advised || !!mergedItem.from_receipt) &&
    !mergedItem.reconstructed_image_url;
  const reconstructionReasons =
    (mergedItem.reconstruction_metadata &&
      mergedItem.reconstruction_metadata.reasons) ||
    [];
          ...
          <div className="closet-banner__content">
            <div className="closet-banner__title-row">
              ...
              <p className="hero-description">
                View, edit, and manage every clothing item in your wardrobe with
                detailed information, smart analysis, and AI-powered styling
                insights.
              </p>
            </div>
          </div>
                ...
                <div className="row gx-3 gy-3">
                        ...
                        <div className="photo-replace-row">
                          {showRepairPhotoCta && (
                            <button
                              ...
                              aria-label={t("item.repairPhoto", {
                                defaultValue: "Repair photo",
                              })}
                            >
                              {reshootingPhoto ? (
                          ...
                          <button
                            ...
                            aria-label={t("itemDetail.photo.takeLabel", {
                              defaultValue: "Take photo",
                            })}
                          >
                            {uploadingPhoto ? (
                  ...
                  <div className="col-md-12" data-testid="item-insights-card">
                    <div className="custm-head">
                      ...
                      <div className="">
                        <h6>
                          {t("itemDetail.stats.label", {
                            defaultValue: "Wardrobe Insights",
                          })}
                        </h6>
                        <p>
                          {t("itemDetail.stats.timesWorn", {
                            defaultValue: "Times Worn",
                          })}
                          :-{item.wear_count || 0}
                        </p>
                      </div>
                      ...
                      {item.price_cents > 0 && (
                        <div className="space-y-1">
                          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
                            {t("itemDetail.stats.costPerWear", {
                              defaultValue: "Cost per Wear",
                            })}
                          </div>
                  ...
                  <div
                    className="col-md-12"
                    data-testid="item-group-views-card"
                  >
                    <div className="custm-body">
                      <div
                        className="caps-label text-muted-foreground truncate max-w-[200px]"
                        title={(() => {
                          const hostItem =
                            currentGroupItems.find(
                              (x) =>
                                x.group_role === "host" || x.id === hostIdState,
                            ) ||
                            currentGroupItems[0] ||
                            item;
                          return hostItem
                            ? hostItem.title || hostItem.name || "Garment Views"
                            : "Garment Views";
                        })()}
                      >
                        {(() => {
                      ...
                      <div className="h-10 mb-4 flex items-center gap-3">
                        {activeViewIdState &&
                        activeViewIdState !== hostIdState &&
                        currentGroupItems.some(
                          (x) => x.id === activeViewIdState,
                        ) ? (
                          <>
                            <Button
                              type="button"
                        ...
                        {currentGroupItems.map((gItem) => {
                          ...
                          return (
                            ...
                            >
                                ...
                                {isHost && (
                                  <div className="absolute inset-x-0 bottom-0 bg-background/80 backdrop-blur-[2px] py-0.5 text-center">
                                    <span className="text-[9px] font-semibold text-[hsl(var(--accent))]">
                                      {t("itemDetail.group.front", {
                                        defaultValue: "Front (Main)",
                                      })}
                                    </span>
                                  </div>
                                )}
                              ...
                              {(() => {
                                const tags = Array.isArray(gItem.tags)
                                  ? gItem.tags
                                  : [];
                                const isFront = tags.some(
                                  (t) => String(t).toLowerCase() === "front",
                                );
                                const isBack = tags.some(
                                  (t) => String(t).toLowerCase() === "back",
                                );
                                const isProfile = tags.some(
                                  (t) => String(t).toLowerCase() === "profile",
                                );
                      ...
                      <div className="flex-1 flex flex-col min-h-0 space-y-6 mt-4">
                        ...
                        <div className="space-y-2 shrink-0">
                          ...
                          >
                            ...
                            <span className="text-start text-xs sm:text-sm whitespace-normal">
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                {filteredCandidates.map((cItem) => (
                                  <div
                                    key={cItem.id}
                                    onClick={() =>
                                      handleSelectClosetItem(cItem.id)
                                    }
                  ...
                  {item.variants && item.variants.length > 0 && (
                    <div>
                      ...
                      <div
                        ...
                        data-testid="item-variant-carousel"
                      >
                  ...
                  <div className="col-md-12">
                    <div className="custm-head">
                      ...
                      <div className="">
                        ...
                        <p>
                          {t("itemDetail.edit.sectionCleanBgDesc", {
                            defaultValue:
                              "Remove background using non-generative matting models",
                          })}
                        </p>
                      </div>
                    </div>
                      ...
                      >
                        {cleaningBackground ? (
                          <>
                            ...
                            {t("itemDetail.cleanBackground.running")}
                          </>
                        ) : (
                          <>
                            <Wand2 className="h-4 w-4 me-2" />
                            {hasReconstruction
                              ? t("itemDetail.cleanBackground.retryCta")
                              : t("itemDetail.cleanBackground.cta")}
                          </>
                        )}
                      </Button>
                      {cleaningBackground && (
                        <div
                          className="space-y-2"
                          data-testid="item-clean-bg-progress"
                        >
                          <Progress
                  ...
                  <div className="col-md-12">
                    <div className="custm-head">
                      ...
                      <div className="">
                        ...
                        <p>
                          {" "}
                          {t("itemDetail.edit.sectionReanalyseDesc", {
                            defaultValue:
                              "Re-run analysis to extract details from the image",
                          })}
                        </p>
                      </div>
                    </div>
                    <div className="custm-body buttonstyle">
                      ...
                      {analyzing && (
                        <div
                          className="space-y-2"
                          data-testid="item-reanalyze-progress"
                        >
                          <Progress
                ...
                <div className="row gx-3 gy-3">
                  ...
                  <div className="col-md-12">
                    <div className="custm-head">
                      ...
                      <div className="">
                        ...
                        <p>
                          {t("itemDetail.edit.sectionIdentityDesc", {
                            defaultValue:
                              "Item title, name, brand, and description details",
                          })}
                        </p>
                      </div>
                    </div>
                    <div className="custm-body">
                      <div className="row gx-3 gy-3">
                        ...
                        <div className="col-md-12">
                          <div className="field-set">
                            <label htmlFor="f-caption">
                              {t("itemDetail.edit.caption")}
                            </label>
                            <Textarea
                              id="f-caption"
                              value={form.caption}
                  ...
                  <div className="col-md-12">
                    <div className="custm-head">
                      ...
                      <div className="">
                        ...
                        <p>
                          {t("itemDetail.edit.sectionTaxonomyDesc", {
                            defaultValue:
                              "Category, item type, gender, and aesthetic styles",
                          })}
                        </p>
                      </div>
                    </div>
                    <div className="custm-body">
                      <div className="row gx-3 gy-3">
                        ...
                        <div className="col-md-6">
                          <div className="field-set">
                            <label htmlFor="f-sub">
                              {t("itemDetail.edit.subCategory")}
                            </label>
                            <Input
                              id="f-sub"
                              value={form.sub_category}
                              onChange={(e) =>
                                setField("sub_category", e.target.value)
                              }
                            ...
                            />
                            <LocalizedHint
                              raw={form.sub_category}
                              translated={labelForSubCategory(
                                form.sub_category,
                                t,
                              )}
                            />
                        ...
                        <div className="col-md-12">
                          <div className="field-set">
                            <label htmlFor="f-itemtype">
                              {t("itemDetail.edit.itemType")}
                            </label>
                            <Input
                              id="f-itemtype"
                              value={form.item_type}
                              onChange={(e) =>
                                setField("item_type", e.target.value)
                              }
                            ...
                            <LocalizedHint
                              raw={form.item_type}
                              translated={labelForItemType(form.item_type, t)}
                            />
                          </div>
                        </div>
                        <div className="col-md-6">
                  ...
                  <div className="col-md-12">
                    <div className="custm-head">
                      ...
                      <div className="">
                        ...
                        <p>
                          {" "}
                          {t("itemDetail.edit.sectionCompositionDesc", {
                            defaultValue:
                              "Garment size, colors, patterns, and fabric materials",
                          })}
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="row gx-3 gy-3">
                        ...
                        <div className="col-md-6">
                          <div className="field-set">
                            ...
                            <LocalizedHint
                              raw={form.color}
                              translated={labelForColor(form.color, t)}
                            />
                          </div>
                        </div>
                        <div className="col-md-12">
                          <div className="field-set">
                            ...
                            <NullableSelect
                              ...
                              className={`createlisting-select ${
                                !form.pattern
                  ...
                  <div className="col-md-12">
                    <div className="custm-head">
                      ...
                      <div className="">
                        ...
                        <p>
                          {" "}
                          {t("itemDetail.edit.sectionQualityDesc", {
                            defaultValue:
                              "Garment state, wear condition, and repair advice",
                          })}
                        </p>
                      </div>
                    </div>
                    <div className="custm-body">
                      <div className="row gx-3 gy-3">
                        ...
                        <div className="col-md-12">
                          <div className="field-set">
                            <label htmlFor="f-repair">
                              {t("itemDetail.edit.repairAdvice")}
                            </label>
                            <Textarea
                              id="f-repair"
                              value={form.repair_advice}
                              onChange={(e) =>
                                setField("repair_advice", e.target.value)
                              }
                              rows={2}
                  ...
                  <div className="col-md-12">
                    <div className="custm-head">
                      ...
                      <div className="">
                        ...
                        <p>
                          {t("itemDetail.edit.sectionPricingDesc", {
                            defaultValue:
                              "Item purchase or retail pricing and transaction intent",
                          })}
                        </p>
                      </div>
                    </div>
                        ...
                        <div className="col-md-4">
                          <div className="field-set">
                            ...
                            <NullableSelect
                              ...
                              placeholder={t("addItem.currencyPlaceholder", {
                                defaultValue: "USD",
                              })}
                        ...
                        <div className="col-md-4">
                          <div className="field-set">
                            ...
                            <NullableSelect
                              ...
                              onChange={(v) =>
                                setField("marketplace_intent", v || "own")
                              }
                              options={INTENT_OPTIONS}
                              placeholder={t("addItem.sourceTagPlaceholder", {
                                defaultValue: "own",
                              })}
                              testid="item-edit-field-marketplace_intent"
                              format={(o) => labelForIntent(o, t)}
                              className={`createlisting-select ${
                                !form.marketplace_intent
                                  ? "border-red-400 dark:border-red-900 focus:ring-red-500"
                                  : ""
                              }`}
                            />
                  ...
                  <div className="col-md-12">
                    <div className="custm-head">
                      ...
                      <div className="">
                        ...
                        <p>
                          {t("itemDetail.edit.sectionOrganizationDesc", {
                            defaultValue:
                              "Outfit formality level, tags, and cultural styling notes",
                          })}
                        </p>
                      </div>
                    </div>
                        ...
                        <div className="col-md-12">
                          <div className="field-set">
                            ...
                            <ChipList
                              ...
                              placeholder={t(
                                "itemDetail.edit.culturalTagPlaceholder",
                              )}
                  ...
                  <div className="col-md-12">
                    <div className="custombtnmain buttonstyle">
                      <Link
                        to={`/market/create?itemId=${item.id}`}
                        className="custm-btn w-100"
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            <AlertDialogTitle>
                              {t("itemDetail.removeTitle")}
                            </AlertDialogTitle>
                            <AlertDialogDescription>
                              {t("itemDetail.removeBody")}
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>
            ...
            >
              <CardContent className="p-5 space-y-3.5">
                        ...
                        {msg.role === "user" ? (
                          ...
                          <div className="flex flex-col items-start space-y-1.5 max-w-[92%]">
                            <div className="flex items-center gap-1.5">
                              <Badge
                                variant="outline"
                                className="text-[10px] h-4 px-1.5 gap-1 bg-sky-50 dark:bg-sky-950/40 text-sky-700 dark:text-sky-300 border-sky-200"
                              >
                                <Sparkles className="h-2.5 w-2.5" />
                                {t("itemDetail.reanalyze.eyesBadge", {
                                  defaultValue: "The Eyes",
                                })}
                              </Badge>
                              {msg.action_taken === "image_edit" && (
                                <Badge
                                  variant="secondary"
                                  className="text-[9px] h-4 px-1.5 bg-amber-100 text-amber-800 dark:bg-amber-950/50 dark:text-amber-300"
                                >
                                  {t("itemDetail.reanalyze.nanoBananaBadge")}
                                </Badge>
                              )}
                              {msg.action_taken === "metadata_update" && (
                                <Badge
                                  variant="secondary"
                                  className="text-[9px] h-4 px-1.5 bg-emerald-100 text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300"
                                >
                                  {t(
                                    "itemDetail.reanalyze.refreshedAttributes",
                                    { defaultValue: "Refreshed attributes" },
                                  )}
                                </Badge>
                              )}
                            </div>
                              ...
                              {msg.image_url && (
                                <div className="mt-2 rounded-lg border border-border/60 bg-muted/40 p-2 space-y-2">
                                  ...
                                  <Button
                                    ...
                                    onClick={() =>
                                      onApplyReconstructedImage(msg.image_url)
                                    }
                                  ...
                                  >
                                    ...
                                    msg.image_url ? (
                                      <>
                                        ...
                                        {t("itemDetail.reanalyze.applied", {
                              ...
                              {msg.updated_fields && (
                                <div className="flex flex-wrap gap-1 pt-1">
                                  {Object.keys(msg.updated_fields).map((k) => (
                                    <span
                                      key={k}
                                      className="text-[9px] px-1.5 py-0.5 rounded bg-muted font-medium text-muted-foreground"
                                    >
                                      ✓ {k}
                                    </span>
                                  ))}
                ...
                <div className="space-y-1.5">
                  ...
                  <div
                    className="flex flex-wrap gap-1.5"
                    data-testid="item-reanalyze-prompt-chips"
                  >
                    <button
                      type="button"
                      onClick={() =>
                ...
                <div className="relative flex items-center gap-1.5 border border-border/70 rounded-xl bg-background p-1.5 focus-within:ring-2 focus-within:ring-ring">
                  ...
                  <Button
                    ...
                    disabled={
                      !reanalyzePrompt.trim() || reanalyzeChatBusy || analyzing
                    }
                ...
                {reanalyzeChatBusy && (
                  <div
                    ...
                    data-testid="item-reanalyze-chat-progress"
                  >
                    <Progress
                      value={reanalyzeChatProgress}
                    ...
                    <div className="flex items-center justify-between text-[10px] text-muted-foreground">
                      ...
                      <span className="tabular-nums">
                        {Math.round(reanalyzeChatProgress)}%
                      </span>
                    </div>
                  </div>
                )}
          ...
          <div className="md:col-span-2 space-y-4" data-testid="item-edit-form">
            ...
            <Card className="rounded-[calc(var(--radius)+6px)] shadow-editorial border-t-2 border-[hsl(250_95%_70%)]">
              <CardContent className="p-5 space-y-3">
                ...
                <Field label={t("itemDetail.edit.subCategory")} htmlFor="f-sub">
                  ...
                  <LocalizedHint
                    raw={form.sub_category}
                    translated={labelForSubCategory(form.sub_category, t)}
                  />
                </Field>
                <Field
                  label={t("itemDetail.edit.itemType")}
                  htmlFor="f-itemtype"
                >
                  <Input
                    id="f-itemtype"
                ...
                <Field
                  label={t("itemDetail.edit.tradition")}
                  htmlFor="f-tradition"
                >
                  <Input
                    id="f-tradition"
            ...
            <Card className="rounded-[calc(var(--radius)+6px)] shadow-editorial border-t-2 border-[hsl(142_72%_50%)]">
              <CardContent className="p-5 space-y-3">
                ...
                <Field
                  label={t("itemDetail.edit.repairAdvice")}
                  htmlFor="f-repair"
                >
                  <Textarea
                    id="f-repair"
            ...
            <AlertDialog open={gatekeeperOpen} onOpenChange={setGatekeeperOpen}>
              <AlertDialogContent data-testid="item-edit-gatekeeper-dialog">
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    {t("itemDetail.gatekeeper.body", {
                      defaultValue:
                        "This host item and its group members have mismatched properties: {{mismatches}}. Are you sure you want to save these changes?",
                ...
                <AlertDialogFooter>
                  <AlertDialogCancel
                    data-testid="item-edit-gatekeeper-cancel"
                    onClick={() => setGatekeeperOpen(false)}
                  >
                    {t("itemDetail.gatekeeper.cancel", {
                      defaultValue: "Cancel",
                    })}
                  </AlertDialogCancel>
                  <AlertDialogAction
                  ...
                  >
                    {t("itemDetail.gatekeeper.confirm", {
            ...
            <div className="fixed bottom-28 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-2.5 py-1.5 rounded-full border border-border bg-card/90 backdrop-blur-lg shadow-xl md:bottom-8 max-w-[calc(100vw-2rem)] shrink-0 animate-[slideUp_0.2s_ease-out]">
              ...
              <Button
                ...
                title={
                  saving
                    ? t("itemDetail.edit.saving")
                    : t("itemDetail.edit.save")
                }
                aria-label={
                  saving
                    ? t("itemDetail.edit.saving")
...
function Field({ label, children, htmlFor, required }) {
  return (
    <div className="space-y-1">
      <Label
        htmlFor={htmlFor}
        className="caps-label text-muted-foreground text-[10px]"
      >
        {label}
        {required ? " *" : ""}
      </Label>
