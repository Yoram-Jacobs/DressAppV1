...
function LocalizedHint({ raw, translated }) {
  ...
  return (
    <div className="text-[12px] text-text-brand font-semibold truncate" data-testid="localized-display-hint">
      {translated}
    </div>
  );
}
      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={`rounded-full border px-3 py-1 text-[12px] font-semibold transition-colors text-text-brand ${on
              ? 'bg-primary-brand text-white border-primary-brand'
              : 'bg-white border-border hover:border-primary-brand hover:text-primary-brand'
              }`}
          >
            ...
            <div className="max-w-[520px]">
              ...
              >
                {t("itemDetail.banner.subtitle", {
                  defaultValue:
                    "Fine-tune every detail The Eyes picked up — fix the photo, adjust attributes, and get this piece ready for your closet or the marketplace.",
                })}
              </p>
            </div>
          </div>
      ...
      <section className="bg-[var(--accent-beige)] px-[40px] py-[80px] max-[991px]:px-[5px] max-[991px]:py-[40px]">
        ...
        <div className="fixed bottom-28 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-2.5 py-1.5 rounded-full border border-border bg-white backdrop-blur-lg shadow-xl md:bottom-8 max-w-[calc(100vw-2rem)] shrink-0 animate-[slideUp_0.2s_ease-out]">
          ...
          {isDirty && (
            <Badge
              variant="outline"
              className="rounded-full flex justify-center text-[12px] h-9 w-9 px-2 bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20 whitespace-nowrap"
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="md:col-span-3 space-y-4">
            {/* image-card */}
            <Card className="rounded-[12px] bg-white overflow-hidden shadow-sm border border-border relative">
              <AspectRatio ratio={4 / 3} className="bg-white dark:bg-[#F5F2EB]">
                {preferredImage ? (
                  <img
                    src={preferredImage}
                  ...
                  <div
                    className="w-full h-full flex flex-col items-center justify-center gap-3 text-text-brand bg-gradient-to-br from-muted/50 to-muted/20 p-6 text-center"
                    data-testid="item-detail-no-image"
                  >
                    {item.dpp_data ? (
                      <>
                        <QrCode className="h-10 w-10 text-primary-brand" />
                    ...
                    >
                      <Images className="h-4 w-4 text-primary-brand" />
                ...
                <div className="absolute bottom-3 end-3 inline-flex items-center gap-1.5 flex-wrap justify-end">
                  ...
                  >
                    {uploadingPhoto ? (
                      ...
                      <Camera className="h-3 w-3 text-primary-brand" />
                    )}
                    {t('itemDetail.photo.takeLabel', { defaultValue: 'Take photo' })}
                  </button>
                  <button
                  ...
                  >
                    <Images className="h-3 w-3 text-primary-brand" />
            ...
            <Card className="rounded-[12px] bg-white overflow-hidden shadow-sm border border-border" data-testid="item-insights-card">
              <CardContent className="p-5 space-y-3">
                <div className="text-[14px] text-dark-brand font-bold">{t('itemDetail.stats.label', { defaultValue: 'Wardrobe Insights' })}</div>
                <div className="grid grid-cols-2 gap-4 pt-1">
                  <div className="space-y-1">
                    <div className="text-[12px] font-semibold text-text-brand">{t('itemDetail.stats.timesWorn', { defaultValue: 'Times Worn' })}</div>
                    <div className="text-[18px] font-bold text-primary-brand">
                      {item.wear_count || 0}
                    </div>
                  </div>
                  {item.price_cents > 0 && (
                    <div className="space-y-1">
                      <div className="text-[12px] font-semibold text-text-brand">{t('itemDetail.stats.costPerWear', { defaultValue: 'Cost per Wear' })}</div>
                      <div className="text-[18px] font-bold text-primary-brand">
            ...
            <Card className="rounded-[12px] bg-white overflow-hidden shadow-sm border border-border" data-testid="item-group-views-card">
              <CardContent className="p-5 space-y-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-[14px] text-dark-brand font-bold truncate max-w-[200px]" title={(() => {
                  ...
                  {activeViewIdState && activeViewIdState !== hostIdState && currentGroupItems.some(x => x.id === activeViewIdState) ? (
                    <>
                      ...
                      <Button
                        ...
                        disabled={saving}
                        className="flex-1 h-10 border-border hover:bg-muted text-dark-brand"
                        data-testid="unbind-member-view-btn"
                      >
                        <Unlink className="h-4 w-4 me-2 text-text-brand" />
                        {t('itemDetail.ungroupTitle', { defaultValue: 'Remove from Group' })}
                      </Button>
                    </>
                  ) : (
                    <div className="flex-1 text-center text-[12px] font-semibold text-text-brand italic flex items-center justify-center h-full">
                      {t('common.edit')}
                    </div>
                  )}
                </div>
                <div className="flex items-start gap-3 overflow-x-auto pb-2 pt-1 scrollbar-thin">
                    ...
                    return (
                      ...
                      >
                        <div
                          ...
                          className={`relative flex-shrink-0 group w-24 !h-[130px] rounded-[12px] overflow-hidden border cursor-pointer transition-all shadow-sm ${isActive
                        ...
                        >
                          ...
                          {isHost && (
                            <div className="absolute inset-x-0 bottom-0 rounded-bl-[12px] rounded-br-[12px] bg-white backdrop-blur-[2px] p-1 text-center">
                              <span className="text-[10px] font-bold text-dark-brand block">
                        ...
                        {(() => {
                          ...
                          return (
                            <span
                              className="text-[10px] font-medium text-text-brand"
                  ...
                  <button
                    ...
                    disabled={uploadingPhoto}
                    className="relative flex-shrink-0 w-24 !h-[130px] rounded-[12px] border border-dashed border-border flex flex-col items-center justify-center gap-1 text-text-brand  transition-colors bg-accent-beige"
            ...
            <Dialog open={addOpen} onOpenChange={setAddOpen}>
              <DialogContent className="max-w-lg rounded-[20px] p-5 glassmorphic max-h-[90dvh] flex flex-col">
                <DialogTitle className="text-xl font-bold flex items-center gap-2">
                  <Images className="h-5 w-5 text-primary-brand" />
                  {t('addItem.addPhotos')}
                </DialogTitle>
                <DialogDescription>{t('closet.subtitle')}</DialogDescription>
                <div className="flex-1 flex flex-col min-h-0 space-y-3">
                  {/* Option 1: Upload New View */}
                  <div className="space-y-2 shrink-0">
                    <h3 className="text-[14px] font-semibold text-text-brand">{t('scanner.tabFile')}</h3>
                    <Button
                      type="button"
                      onClick={() => {
                        setAddOpen(false);
                      ...
                      className=""
                    >
                      <Camera className="h-5 w-5 shrink-0 text-white" />
                  ...
                  <div className="flex-1 flex flex-col min-h-0 space-y-3">
                    <div className="flex items-center justify-between shrink-0">
                      <h3 className="text-[14px] font-semibold text-dark-brand">{t('home.openCloset')}</h3>
                      <span className="text-[12px] text-text-brand">
                        {t('closet.selectedCount', { count: filteredCandidates.length })}
                      </span>
                    </div>
                    {/* Search bar inside picker */}
                    <div className="relative shrink-0">
                      <Search className="absolute start-3 top-1/2 -translate-y-1/2 h-4 w-4 text-text-brand" />
                      <Input
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-2">
                          {filteredCandidates.map((cItem) => (
                            <div
                              ...
                              className="group flex flex-col rounded-[12px] border border-border overflow-hidden cursor-pointer hover:border-primary-brand transition-all  shadow-sm"
                            >
                              <div className="aspect-[3/4] bg-accent-beige relative overflow-hidden flex items-center justify-center">
                                <img
                              ...
                              <div className="p-2 text-start border-t border-border bg-white">
                                <p className="text-[12px] font-bold text-dark truncate">
                                  {cItem.title || cItem.name}
                                </p>
                                <p className="text-[10px] font-semibold text-text-brand truncate mt-0.5">
            ...
            {item.variants && item.variants.length > 0 && (
              <div>
                <div className="caps-label text-text-brand mb-2">
            ...
            <Card className="rounded-[12px] bg-white overflow-hidden shadow-sm border border-border" data-testid="item-clean-bg-card">
              ...
              <CardContent className="p-5 space-y-3">
                <p className="text-[14px] text-text-brand font-semibold">
                ...
                >
                  {cleaningBackground ? (
                    ...
                    <><Wand2 className="h-4 w-4" />{hasReconstruction ? t('itemDetail.cleanBackground.retryCta') : t('itemDetail.cleanBackground.cta')}</>
                  )}
                </Button>
                {cleaningBackground && (
                  <div className="space-y-2" data-testid="item-clean-bg-progress">
                    ...
                    />
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-[12px] text-text-brand italic font-semibold">
                        {t('itemDetail.cleanBackground.progressHint')}
                      </p>
                      <span className="text-[12px] tabular-nums text-text-brand font-bold" data-testid="item-clean-bg-progress-pct">
                        {Math.round(cleanBackgroundProgress)}%
                      </span>
                    </div>
                  </div>
                )}
                <p className="text-[12px] font-semibold text-text-brand italic font-semibold">
            ...
            <Card className="rounded-[12px] bg-white overflow-hidden shadow-sm border border-border" data-testid="item-reanalyze-card">
              ...
              <CardContent className="p-5 space-y-3">
                      ...
                      <div key={idx} className="space-y-3">
                        {msg.role === 'user' ? (
                          <div className="flex justify-end">
                            <div className="bg-primary-brand text-white rounded-[12px] rounded-br-none px-3 py-2 max-w-[85%] shadow-sm leading-relaxed">
                              {msg.content}
                            </div>
                          </div>
                        ) : (
                          <div className="flex flex-col items-start space-y-1.5 max-w-[92%]">
                            <div className="flex items-center gap-1.5">
                              <Badge variant="outline" className="text-[10px] px-1.5 gap-1 bg-sky-50 dark:bg-sky-950/40 text-sky-700 dark:text-sky-300">
                              ...
                              {msg.action_taken === 'metadata_update' && (
                                <Badge variant="secondary" className="text-[10px] px-1.5 bg-emerald-100 text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300">
                                  {t('itemDetail.reanalyze.refreshedAttributes', { defaultValue: 'Refreshed attributes' })}
                                </Badge>
                              )}
                            </div>
                            <div
                              className={`rounded-[12px] rounded-bl-none p-3 shadow-sm border  leading-relaxed space-y-2 w-full ${msg.error
                                ? 'bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-900 text-red-900 dark:text-red-200'
                                : 'bg-card border-border/60 text-dark-brand'
                                }`}
                            >
                              ...
                              {msg.image_url && (
                                <div className="bg-accent-beige rounded-[12px] p-3 space-y-2 flex items-center justify-center flex-col">
                                  ...
                                  <Button
                                    type="button"
                                    size="sm"
                                    variant={form.reconstructed_image_url === msg.image_url ? "" : "default"}
                                    className="!gap-1"
                              ...
                              {msg.updated_fields && (
                                <div className="flex flex-wrap gap-1 pt-1">
                                  {Object.keys(msg.updated_fields).map((k) => (
                                    <span key={k} className="text-[10px] px-1.5 py-0.5 rounded-full bg-primary-shadow font-semibold text-primary-brand">
                                      ✓ {k}
                                    </span>
                                  ))}
                                </div>
                ...
                {/* Prompt Input Box */}
                <div className="relative flex items-center gap-1.5 border border-border rounded-[12px] bg-primary-shadpw px-1 py-1">
                  <Input
                  ...
                  {sttSupported.current && (
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      className={`h-7 w-7 rounded-lg shrink-0 ${reanalyzeDictating
                  ...
                  <Button
                    type="button"
                    size="icon"
                    className="h-7 w-7 rounded-lg shrink-0 bg-primary-brand text-white"
                ...
                {analyzing && (
                  <div className="space-y-2" data-testid="item-reanalyze-progress">
                    ...
                    />
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-[11px] text-text-brand italic">
                        {t('itemDetail.reanalyze.progressHint')}
                      </p>
                      <span
                        className="text-[12px] tabular-nums text-text-brand font-bold"
          ...
          <div className="md:col-span-2 space-y-4" data-testid="item-edit-form">
            {/* Identity */}
            <Card className="rounded-[12px] bg-white overflow-hidden shadow-sm border border-border">
              <div className="flex items-center gap-3 p-5 border-b border-border">
                <div className="p-3 rounded-full bg-primary-shadow text-primary-brand dark:bg-[hsl(271_30%_18%)] dark:text-[hsl(271_81%_70%)] shrink-0">
            ...
            {/* Bottom actions */}
            <div className="grid grid-cols-2 gap-3">
              <Button asChild data-testid="item-list-for-sale">
                <Link to={`/market/create?itemId=${item.id}`} className='!py-3'>
                  <Store className="h-4 w-4" />{t('itemDetail.listForSale')}
                </Link>
              </Button>
              <AlertDialog>
...
function Field({ label, children, htmlFor, required }) {
  return (
    <div className="">
      <Label htmlFor={htmlFor} className="text-[14px] font-semibold text-text-brand mb-[15px] block leading-none">
        {label}{required ? ' *' : ''}
      </Label>
      {children}
    </div>
