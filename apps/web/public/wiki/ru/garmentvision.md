...
- **Движок генеративной реставрации (`gemini_image_service.py`):**
  - **Инпейтинг / Аутпейтинг (`edit`):** Передает фрагмент изображения и структурированный промпт в `gemini-3.1-flash-lite-image`, сохраняя текстуру ткани, узор и цвет при достраивании недостающей геометрии.
  - **Студийная генерация (`generate`):** Формирует запрос к `gemini-3.1-flash-lite-image` с подробными метаданными (тип вещи, материал, цвет, фурнитура, вырез) для создания идеального каталожного снимка на нейтральном светлом фоне.
