...
export function CampaignDetailScreen() {
  ...
  const handlePause = async () => {
    ...
    try {
      ...
      Alert.alert(t('common.error', { defaultValue: 'Error' }), (err as { message?: string })?.message);
  ...
  const handleCancel = () => {
    Alert.alert(
      t('campaigns.cancelTitle', { defaultValue: 'Cancel campaign' }),
      t('campaigns.cancelMsg', { defaultValue: 'This cannot be undone. The campaign will be permanently cancelled.' }),
      [
        { text: t('common.cancel', { defaultValue: 'Cancel' }), style: 'cancel' },
        {
          text: t('campaigns.confirmCancel', { defaultValue: 'Cancel campaign' }), style: 'destructive', onPress: async () => {
            setActing(true);
            try {
  ...
  const handleShare = async () => {
    try {
      ...
      await Share.share({
        title: campaign?.title || t('campaigns.create.title', { defaultValue: 'Campaign' }),
        message: t('campaigns.shareMessage', {
          defaultValue: 'Check out this campaign: {{title}}',
  ...
  if (!campaign) return <SafeAreaView style={[s.root, s.center]}><Text style={s.emptyText}>{t('common.notFound', { defaultValue: 'Not found' })}</Text></SafeAreaView>;
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        {(campaign.views_count != null || campaign.saves_count != null || campaign.shares_count != null) && (
          <View style={[s.statsRow, { flexDirection: isRtl ? 'row-reverse' : 'row' }]}>
            {campaign.views_count != null && <View style={s.statItem}><Text style={s.statValue}>{campaign.views_count}</Text><Text style={s.statLabel}>{t('campaigns.views', { defaultValue: 'Views' })}</Text></View>}
            {campaign.saves_count != null && <View style={s.statItem}><Text style={s.statValue}>{campaign.saves_count}</Text><Text style={s.statLabel}>{t('campaigns.saves', { defaultValue: 'Saves' })}</Text></View>}
            {campaign.shares_count != null && <View style={s.statItem}><Text style={s.statValue}>{campaign.shares_count}</Text><Text style={s.statLabel}>{t('campaigns.shares', { defaultValue: 'Shares' })}</Text></View>}
          </View>
        )}

        {/* Meta grid */}
        {campaign.budget_cents != null && <InfoRow icon="💰" label={t('campaigns.budget', { defaultValue: 'Budget' })} value={`${(campaign.budget_cents / 100).toFixed(0)} ${campaign.currency ?? '€'}`} colors={colors} />}
        {campaign.start_date && <InfoRow icon="📅" label={t('campaigns.startDate', { defaultValue: 'Start' })} value={campaign.start_date.slice(0, 10)} colors={colors} />}
        {campaign.end_date   && <InfoRow icon="🏁" label={t('campaigns.endDate', { defaultValue: 'End' })} value={campaign.end_date.slice(0, 10)} colors={colors} />}
        {campaign.location   && <InfoRow icon="📍" label={t('campaigns.location', { defaultValue: 'Location' })} value={campaign.location} colors={colors} />}
        {campaign.target_audience && <InfoRow icon="🎯" label={t('campaigns.audience', { defaultValue: 'Audience' })} value={campaign.target_audience} colors={colors} />}

        {/* Description */}
        {campaign.description && (
          <View style={s.descBlock}>
            <Text style={s.descTitle}>{t('campaigns.description', { defaultValue: 'Description' })}</Text>
        ...
        <View style={s.actions}>
          {status === 'active' && (
            <>
              <ActionBtn label={`⏸ ${t('campaigns.pause', { defaultValue: 'Pause' })}`} onPress={handlePause} loading={acting} colors={colors} />
              <ActionBtn label={`📤 ${t('campaigns.share', { defaultValue: 'Share' })}`} onPress={handleShare} colors={colors} variant="secondary" />
            </>
          )}
          {status === 'paused' && (
            <>
              <ActionBtn label={`▶ ${t('campaigns.resume', { defaultValue: 'Resume' })}`} onPress={handleResume} loading={acting} colors={colors} />
              <ActionBtn label={`✕ ${t('campaigns.cancel', { defaultValue: 'Cancel' })}`} onPress={handleCancel} loading={acting} colors={colors} variant="danger" />
            </>
          )}
        </View>
      </ScrollView>
